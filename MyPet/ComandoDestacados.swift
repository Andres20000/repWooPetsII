...
class ComandoDestacados
{
    class func  destacarPublicacion(publicacion:PublicacionOferente, newVersion:Int)
    {
        ...
        var ref  = Database.database().reference().child("productos/" +  publicacion.idPublicacion! + "/destacado")
        ref.setValue(true)
        
        // 2. Aumentamos la version de la imagen
        ref  = Database.database().reference().child("imagenes/" + publicacion.idPublicacion! +  "/" + imagen!.idImagen)
        ...
        refS.putData(imagen!.imageData!, metadata: metadata)
        
        let uploadTask = refS.putData(imagen!.imageData!, metadata:  metadata) { metadata, error in
            if (error != nil) {
