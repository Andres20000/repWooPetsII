...
class ComandoPreguntasOferente
{
    class func getTodasMisPreguntas()
    {
        ...
        ref.observe(.childAdded, with: {(snap) -> Void in
            ...
            for pregunta in model.preguntas
            {
                if pregunta.idPregunta == pregu.idPregunta
                {
                    model.preguntas.remove(at: i)
                }
    ...
    {
        var ref  = Database.database().reference().child("preguntas/" + idPregunta + "/respuesta")
        ref.setValue(respuesta)
        
        let nowDate = NSDate() as Date
        
        ref  = Database.database().reference().child("preguntas/" + idPregunta + "/fechaRespuesta")
        ref.setValue(nowDate.fechaCompletaString())
    }
    ...
    class func getPreguntasFrecuentes()
    {
        ...
        let ref  = Database.database().reference().child("preguntasFrecuentes/")
